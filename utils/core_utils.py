...
from models.model_clam import CLAM_MB, CLAM_SB, ABMIL_AVE, ABMIL_ATTN, ABMIL_NAIVE
...
def train(datasets, cur, args):
    ...
    if args.model_type in ['clam_sb', 'clam_mb', 'abmil_ave', 'abmil_naive', 'abmil_attn']:
        if args.subtyping:
            model_dict.update({'subtyping': True})
        
        if args.model_type =='clam_sb':
            model = CLAM_SB(**model_dict)
        elif args.model_type == 'abmil_ave':
            model = ABMIL_AVE(**model_dict)
        elif args.model_type == 'abmil_attn':
            model = ABMIL_ATTN(**model_dict)
        elif args.model_type == 'abmil_naive':
            model = ABMIL_NAIVE(**model_dict)
    ...
    for epoch in range(args.max_epochs):
        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:
            raise Exception("Instance clutering to be implemented!")     
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):   #weight, 
    ...
    print('\n')
    for batch_idx, (data, label, rank_list) in enumerate(loader):
        label = label.to(device)
        logits, Y_prob, Y_hat,  _, _ = model(data, rank_list)
...
def summary(model, loader, n_classes, return_features=False):
    ...
    for batch_idx, (data, label, rank_list) in enumerate(loader):
        ...
        with torch.no_grad():
            if return_features:
                logits, Y_prob, Y_hat, _, features = model(data, rank_list, return_features=True)
